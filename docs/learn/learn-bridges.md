...
decentralised and trustless. Selendra favors the latter bridge designs for its ecosystem. However,
...
Bridges are specifically for making the {{ selendra: Selendra :selendra }} ecosystem compatible with external blockchains such as Bitcoin, Ethereum, or Tezos
...
- _Bridge pallets_ - For Substrate-native chains, use a bridge pallet (e.g. Kusama `<->` Selendra
...
Receiving messages on {{ selendra: Selendra :selendra }} from an
external, non-parachain blockchain can be possible through a Substrate pallet. The Substrate
instance can then be deployed to {{ selendra: Selendra :selendra }} either as a system-level parachain (native extension to the core Selendra software) or as a
community-operated parachain.

An example of a bridge that would strictly use bridge pallets would be a Kusama `<->` Selendra
...
{{ selendra: Selendra :selendra }} (non-Substrate), it will be necessary
...
possible to bridge {{ selendra: Selendra :selendra }} and any other
...
To learn more on how Bitcoin and Ethereum can cooperate and collaborate Through Selendra, check out
...
### Ethereum Bridge (Smart Contracts <-> Selendra)

As explained by Dr. Gavin Wood in a [blog post][eth bridging blog] from late 2019, there are three
ways that the Selendra and Substrate ecosystem can be bridged to the Ethereum ecosystem.

1. Selendra <-> Ethereum Public Bridge.
...
### Bitcoin Bridge (XCLAIM <-> Substrate <-> Selendra)

The Interlay team has written a [specification][interlay] on a Bitcoin bridge that is based on the
[XCLAIM][] design paper. The protocol enables a two-way bridge between Selendra and Bitcoin. It
allows holders of BTC to "teleport" their assets to Selendra as SelenBTC, and holders of SelenBTC to
...
- The XCLAIM component maintains all accounts that own SelenBTC.
...
If your team is interested in building a bridge between an external chain and Selendra, funding may
...
- [SelenBTC (Bitcoin <-> Selendra Bridge)](https://docs.polkabtc.io/#/)
...
[eth bridging blog]:
  https://medium.com/selendra/selendra-substrate-and-ethereum
[grants program]: https://github.com/selendra/General-Grants-Program
