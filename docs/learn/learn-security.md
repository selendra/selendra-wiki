...
Security is independent of the number of parachains that are connected to the Selendra Relay Chain.
...
of SEL that are bonded and staked. The greater the number of SEL staked by honest validators and
nominators, the higher the minimum amount of SEL an attacker would need to acquire a validator slot.
...
secured by the Selendra Relay Chain validator set. However, in some cases (which are considered more
