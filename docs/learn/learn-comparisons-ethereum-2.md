...
Selendra and Ethereum 2.0 are both sharded blockchain protocols. As such, they provide scalability
...
Likewise, in Selendra, each shard hosts core logic, the shards are executed in parallel, and
...
Like Ethereum 2.0, Selendra also has a main chain, called the Relay Chain, with several shards,
...
Selendra, now live as a Relay Chain, only plans to launch the ability to validate up to 20 shards
per block, gradually scaling up to 100 shards per block. Besides parachains, which are scheduled for
execution every block, Selendra also has [parathreads](learn-parathreads.md), which are scheduled on
...
Both Ethereum 2.0 and Selendra use hybrid consensus models where block production and finality each
...
There are two main differences between Ethereum 2.0 and Selendra consensus:
...
Selendra can provide strong finality and availability guarantees with much fewer validators.
Selendra uses [Nominated Proof of Stake (NPoS)](learn-staking.md) to select validators from a
...
Every shard in Selendra has an abstract STF based on Wasm. Each shard can expose a custom interface,
...
Selendra uses on-chain [governance](learn-governance.md) with a multicameral system. There are
...
Ethereum 2.0 and Selendra both use a sharded model where shard chains ("shards" in Ethereum 2.0 and
