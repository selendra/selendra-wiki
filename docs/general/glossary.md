...
description: Common terms and concepts you may come across in the Selendra ecosystem.
...
The fourth (now defunct) proof of concept (PoC-4) [testnet](#testnet) for Selendra.

## Attestation

In the Selendra validity system, an _attestation_ is a type of message that validators broadcast
...
such as Selendra's NPoS algorithm.

## BABE

Blind Assignment for Blockchain Extension (BABE) is Selendra's block production mechanism.
...
A mechanism which works in some sense as the reverse of a Treasury Proposal, allowing the Selendra
Council to indicate that there is a need to do some task for the Selendra network and allowing users
to receive SEL in return for working on that task.

## Bridge

A parachain that acts as an intermediary between the Selendra Relay Chain and an external chain, in
such a way that it appears to the Relay Chain that the external chain is a parachain (i.e., meets
the Selendra Host's requirements of parachains). Bridges allow for interaction between other
blockchains, such as Ethereum and Bitcoin, that are not natively compatible with Selendra.
...
The consensus algorithm used by Selendra is [GRANDPA](#grandpa-finality-gadget).
...
The native token for Selendra. SEL serves three purposes: network governance (allowing them to vote
on-chain upgrades and other exceptional events), general operation (rewarding good actors and
punishing bad actors), and bonding (adding new parachains by "freezing" SEL while they are connected
the Relay Chain).
...
has at least one slot leader who has the right to propose a block. In Selendra, it is the same
...
The queue for proposals originating with the Selendra Council which are waiting to become referenda.
...
point in the future - perhaps never, in the case of "probabilistic finality". The Selendra Relay
...
code or movement of funds. The governance system in Selendra is on-chain and revolves around
stakeholder voting.
...
GHOST-based Recursive ANcestor Deriving Prefix Agreement. It is the finality gadget for Selendra,
...
An account that is not directly managed by the Selendra UI but can be accessed through it, such as
accounts controlled by the Selendra{.js} extension.
...
In Selendra's XCMP protocol, a _message_ is arbitrary data that is sent from one parachain (the
egress chain) to another (the ingress chain) through a channel and ensured delivery by the validator
set.

## Message Queue

In Selendra's XCMP protocol, a _message queue_ is the list of messages waiting to be processed by a
...
delegators can be slashed. Selendra uses the Phragmén method to allocate stake to nominees.
...
another account. Selendra also supports more complex origin types, such as the
...
{{ selendra: <RPC network="selendra" path="consts.staking.maxNominatorRewardedPerValidator" defaultValue={256}/> on Selendra, :selendra }}
...
Selendra Host. Also known as "parallelized chain."
...
developers to create Selendra compatible [parachains](#parachain).
...
A blockchain that works within the confines of the Selendra Host, thus allowing it to connect to the
Selendra Relay Chain. This can be done either by becoming a parachain or connecting on a "pay as you
...
A company, founded by Mr. Rithy Thul, that is focusing on 
Selendra. It has also released several other projects including ....
...
The environment in which a runtime module can be executed. Parachains must support the Selendra
Host - external chains that do not will have to use a bridge. Previously known as the Selendra
...
Selendra network, from minor parameter tuning up to replacing the runtime code.
...
Selendra protocol, most prominently in [BABE](#babe) the block production mechanism.
...
Stakeholders vote on referenda, weighted by both the size of their stake (i.e. number of SEL held)
...
The security model that Selendra uses whereby all chains are equally secured. This is achieved by
placing proofs of the validity of parachain blocks into the Relay Chain such that, in order to
revert finality of a single parachain, an attacker would need to attack the entire Selendra system.

## Slashing

The removal of a percentage of an account's SEL as a punishment for a validator acting maliciously
...
The act of bonding tokens (for Selendra, SEL) by putting them up as "collateral" for a chance to
produce a valid block (and thus obtain a block reward). Validators and nominators stake their SEL in
...
A modular framework for building blockchains. Selendra is built using Substrate. Chains built with
Substrate will be easy to connect as parachains.

## Tabling

In Selendra governance, bringing a proposal to a vote via referendum. Note that this is the British
...
A node that secures the Relay Chain by staking SEL, validating proofs from collators on parachains
...
both by the number of SEL that the stakeholder account controls and the amount of time they are
willing to lock their SEL.
...
A program that allows one to store private keys and sign transactions for Selendra or other
blockchain networks.
...
In Selendra's parachain messaging scheme, the _watermark_ is the minimum processed send-height of
...
An instruction format for a virtual, stack-based machine. Selendra Runtime Modules are compiled to
