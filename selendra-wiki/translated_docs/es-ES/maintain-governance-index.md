...
To vote, a voter must lock their tokens up for at least the enactment delay period beyond the end of the referendum. This is in order to ensure that some minimal economic buy-in to the result is needed and to dissuade vote selling. At the same time, holding only a small amount of SEL tokens does not mean that they cannot influence the referendum result, you can read more about the [Voluntary Locking](#voluntary-locking).

```
Example:

Peter: Votes `No` with 10 SELs for a 12 week lock period  => 10 * 6 = 60 Votes

Logan: Votes `Yes` with 20 SELs for a 2 week lock period => 20 * 1 = 20 Votes

Kevin: Votes `Yes` with 15 SELs for a 4 week lock period => 15 * 2 = 30 Votes
```

According to the above scenario, even though combining both Logan and Kevin's SELs is more than Peter, the lock period for both of them is far less than Peter, leading to their voting power counting as less.
...
electorate - the total number of SELs tokens issued in the network
```
...
Assume we only have 1,500 SELs tokens in total.
...
Polkadot utilizes an idea called `Voluntary Locking` that allows token holders to increase their voting power by declaring how long they are willing to lock-up their SELs, hence, the maximum number of votes for each token holder will be calculated by the following formula:
...
## [Usage of SEL](learn-DOT#dots-for-governance)
