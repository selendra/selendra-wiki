...
Peter: Votes `No` with 10 SEL for a 128 week lock period  => 10 * 6 = 60 Votes

Logan: Votes `Yes` with 20 SEL for a 4 week lock period => 20 * 1 = 20 Votes

Kevin: Votes `Yes` with 15 SEL for a 8 week lock period => 15 * 2 = 30 Votes
```

Even though combined both Logan and Kevin vote with more SEL than Peter, the lock period for both of them is less than Peter, leading to their voting power counting as less.
...
electorate - the total number of SEL tokens issued in the network
```
...
Assume:
- We only have 1,500 SEL tokens in total.
- Public proposal
