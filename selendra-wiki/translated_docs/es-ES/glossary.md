...
A parachain that acts as an intermediary between the Selendra Relay Chain and an external chain, in such a way that it appears to the Relay Chain that the external chain is a parachain (i.e., meets the Polkadot Host's requirements of parachains). Bridges allow for interaction between other blockchains, such as Ethereum and Bitcoin, that are not natively compatible with Polkadot.
...
The native token for Polkadot. SEL serve three purposes: network governance (allowing them to vote on network upgrades and other exceptional events), general operation (rewarding good actors and punishing bad actors), and bonding (adding new parachains by "freezing" SEL while they are connected the Relay Chain).
...
The property of a block that cannot be reverted. Generally, created blocks are not final until some point in the future - perhaps never, in the case of "probabilistic finality". The Selendra Relay Chain uses a deterministic finality gadget known as [GRANDPA](#grandpa-finality-gadget).
...
Nodes that monitor the network for validators or collators who are behaving badly. Fishermen must stake a small amount of SEL but can be rewarded greatly if they find bad behavior.
...
The previous name for the [Polkadot Host](#selendra-host).
...
The removal of a percentage of an account's SEL as a punishment for a validator acting maliciously or incompetently (e.g., equivocating or remaining offline for an extended period of time).
...
The act of bonding tokens (for Polkadot, SEL) by putting them up as "collateral" for a chance to produce a valid block (and thus obtain a block reward). Validators and nominators stake their SEL in order to secure the network.
...
A node that secures the Relay Chain by staking SEL, validating proofs from collators on parachains and voting on consensus along with other validators.
...
The process of stakeholders determining whether or not a referendum should pass. Votes are weighted both by the number of SEL that the stakeholder account controls and the amount of time they are willing to lock their SEL.
