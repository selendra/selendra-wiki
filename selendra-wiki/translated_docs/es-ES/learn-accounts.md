...
When you generate an account (address), you only generate a _key_ that lets you access it. The account does not exist yet on-chain. For that, it needs the existential deposit: 0.001666666667 KSM (on Kusama) or 1 SEL (on Polkadot mainnet).
...
>
> - Existential deposit is equal to 0.01 WND (Westies; Westend's native coin) instead of 1 SEL.
> - The multisignature transaction deposit is equal to ~1 WND instead of ~20.2 SEL.
>
> The photos below reflects values in WND, but instructions are the same for SEL.
...
- SEL to deposit into the multisig address.
- ~20.2 SEL refundable deposit to send a multisig tarnsaction. This needs to be in the address that initiates a multisignature transaction (in this example, Alice).
