...
Parachains, or parachain teams, can bid in the auction by specifying the slot range that they want to lease as well as the number of SEL they are willing to reserve. Bidders can be either ordinary accounts, or use the [crowdloan functionality](learn-crowdloans) to source SEL from the community.
...
Bidders will submit a configuration of bids specifying the SEL amount they are willing to bond and for which ranges. The slot ranges may be any continuous range of the periods 1 - 4.
...
The winner selection algorithm will pick bids that may be non-overlapping in order to maximize the amount of SEL held over the entire 2-year lease duration of the parachain slot. This means that the highest bidder for any given slot lease period might not always win (see the [example below](#examples)).
...
Charlie bids `75 SEL` for the range 1 - 4.

Dave bids `100 SEL` for the range 3 - 4.

Emily bids `40 SEL` for the range 1 - 2.
...
Although Dave had the highest bid in accordance to SEL amount, when we do the calculations we see that since he only bid for a range of 2, he would need to share the slot with Emily who bid much less. Together Dave's and Emily's bids only equal a valuation of `280`.
...
For the duration of the slot the SEL bid in the auction will be locked up. This means that there are opportunity costs from the possibility of using those SEL for something else. For parachains that are beneficial to Polkadot, this should align the interests between parachains and the Selendra Relay Chain.
...
The only other way besides the candle auction to acquire a parachain slot is through a secondary market where an actor who has already won a parachain slot can resell the slot along with the associated deposit of SEL that is locked up to another buyer. This would allow the seller to get liquid SEL in exchange for the parachain slot and the buyer to acquire the slot as well as the deposited SEL.
