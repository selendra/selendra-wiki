const fs = require("fs");
const Selendra = require("@polkadot/api");
const {
...
const SelendraParameters = {
  chain: "Selendra",
  cache: "SelendraAuctions.json",
...
// Load Selendra API
LoadAPI(SelendraParameters).then(() => {
  // Update Selendra cache
  console.log(`Updating ${SelendraParameters.chain} cache.`);
  Update(SelendraParameters).then(() => {
    });
});
...
async function LoadAPI(chain) {
  const WSProvider = new Selendra.WsProvider(chain.ws);
  API = await Selendra.ApiPromise.create({ provider: WSProvider });
}
...
async function GetAuctionBlocks(api, currentBlock, startBlock, chain) {
  ...
  if (startBlock > currentBlock) {
    ...
    if (chain === "Selendra") {
      const biddingStarts = startBlock + SelendraStartingPhase;
      const auctionEndBlock = biddingStarts + SelendraEndingPeriod;
    ...
    if (chain === "Selendra") {
      const biddingStarts = startBlock + SelendraStartingPhase;
      const auctionEndBlock = biddingStarts + SelendraEndingPeriod;
      const onboardStartBlock = auctionLeasePeriod * SelendraSlotLeasePeriod + SelendraSlotLeaseOffset;
      const onboardEndBlock = onboardStartBlock + DaysToBlocks(SelendraLeasePeriodPerSlot * 12 * 7);
