...
import { SelendraAuctions, FutureBlock } from './utilities/auctionVariables';
...
function AuctionSchedule() {
	...
	useEffect(async () => {
		...
		if (title === "Parachain Slot Auctions · Selendra Wiki") {
			const chain = "Selendra";
			const wsProvider = new WsProvider("wss://rpc.selendra.org");
			await LoadOptions(SelendraAuctions, wsProvider);
			await SetCurrentBlockData();
			[ChainState.AuctionIndex, ChainState.AuctionStatus] = GetCurrentOrNextAuction(chain, SelendraAuctions, ChainState.BlockNumber);
			await LoadBlockCacheThenUpdate(chain, SelendraAuctions, setAuctions, { target: { value: ChainState.AuctionIndex } });
		}
		else {
...
function Render(chain, auctions, setAuctions, index) {
	...
	if (chain === "Polkadot") {
		explorerUrl = "https://explorer.selendra.org/block/";
	}
	...
	const content = <div>
		...
		<select 
			...
			style = {{ border: '2px solid #03A9F4', height: '40px' }}
		>
